    ...
    with PdfPages(output_pdf.path) as pdf:
        for node_name, node_df in df.groupby('node'):
            ...
            for step in range(future_steps):
                ...
                X_input = pd.DataFrame({col: [last_known[col].iloc[-1]] for col in FEATURE_COLUMNS})
            ...
            plt.plot(node_df['timestamp'], node_df['cpu_usage_mean'], label='Historical CPU usage (mean)', color='blue')
            x_axis = pd.date_range(start=last_known['timestamp'].iloc[2], periods=future_steps+1, freq='30min')[1:]
            plt.plot(x_axis, predictions, label='Predicted CPU usage (mean)', color='orange', marker='o')
            plt.plot(last_known['timestamp'].iloc[:3], last_known['cpu_usage_mean'].iloc[:3], label='Last known CPU values', color='green', marker='x')
...
def classify_cpu_usage(input_csv: Input[Dataset], output_pdf: Output[Dataset]):
    ...
    declared_cpu_map = {'Frontend UI': 2, 'Backend Service': 2.5, 'Database': 2.5}
...
def what_if_simulator(input_csv: Input[Dataset], input_model: Input[Dataset], output_pdf: Output[Dataset]):
    ...
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    models = joblib.load(input_model.path)
    scenarios = {"baseline": 1.0, "scale_up": 1.5, "scale_down": 0.7}
        ...
        for node_name, node_df in df.groupby('node'):
            ...
            for scenario, factor in scenarios.items():
                ...
                for step in range(future_steps):
                    ...
                    X_input = pd.DataFrame({col: [last_sim[col].iloc[-1]] for col in FEATURE_COLUMNS})
...
# Step 7: Aggregate Nodes and Predict with XGBoost
@dsl.component(base_image="python:3.10")
def aggregate_nodes_and_predict(
    ...
    import subprocess
    subprocess.run([
        "pip", "install", "pandas", "numpy", "matplotlib", "joblib", "xgboost==2.0.3"
    ], check=True)
    ...
    last_known_per_node = {
        node_name: node_df.tail(3).copy().reset_index(drop=True)
        for node_name, node_df in df.groupby('node')
    ...
    for step in range(future_steps):
        ...
        for node_name, last_known in last_known_per_node.items():
            ...
            new_row['cpu_usage_mean'] = y_pred
            new_row['timestamp'] = next_ts
            last_known_per_node[node_name] = pd.concat([last_known, new_row.to_frame().T], ignore_index=True)

        predictions_per_scenario['baseline'].append((next_ts, agg_pred))
        agg_df = pd.concat([agg_df, pd.DataFrame({'timestamp':[next_ts],'cpu_usage_mean':[agg_pred]})], ignore_index=True)


    with PdfPages(output_pdf.path) as pdf:
        plt.figure(figsize=(14,6))
        plt.plot(df.groupby('timestamp')['cpu_usage_mean'].sum(), label="Historical aggregated CPU", color="blue")
        pred_ts, pred_vals = zip(*predictions_per_scenario['baseline'])
        plt.plot(pred_ts, pred_vals, label="Predicted aggregated CPU", color="orange", marker="o")
    ...
    print("Aggregated node prediction PDF generated successfully.")
...
def kcd_Bulgaria():
    ...
    evaluate = evaluate_model(input_csv=preprocess.outputs["output_csv"], input_model=train.outputs["output_model"])
    what_if = what_if_simulator(input_csv=preprocess.outputs["output_csv"], input_model=train.outputs["output_model"])
    aggregate_predict = aggregate_nodes_and_predict(input_csv=preprocess.outputs["output_csv"], input_model=train.outputs["output_model"])


if __name__ == "__main__":
    compiler.Compiler().compile(pipeline_func=kcd_Bulgaria, package_path="KCD_Pipeline_Demo.yaml")
